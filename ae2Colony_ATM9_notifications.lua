...
local notifyPlayer = "EinsamerAkuma"
...
local function logAndDisplay(msg)
  ...
  if msg:find("%[MISSING%]") then
    color = colors.red
  elseif msg:find("%[SENT%]") then
    color = colors.lime
  elseif msg:find("%[CRAFT%]") then
    color = colors.orange
  ...
  if #monitorLines > 120 then
    table.remove(monitorLines, 1)
  end

  if msg:find("%[MISSING%]") and chatBox then

    if not notifiedMissing[msg] then
...
-- MONITOR
local function updateMonitor(monitor, bridge)
  ...
  monitor.setCursorPos(width - 10, 1)
  ...
  for i = math.max(1, #monitorLines - (height - 3)), #monitorLines do

    local line = monitorLines[i]
...
while true do
  ...
  for k,v in pairs(notifiedMissing) do
    if os.clock() - v > 120 then
      notifiedMissing[k] = nil
    end
  end
