...
local scanInterval = 8             -- Probably shouldn't go much lower than 20s...
...
local function setupPeripherals()
  ...
  local bridge = peripheral.find("meBridge") or error("me_bridge missing")
  local colony = peripheral.find("colonyIntegrator") or error("colony_integrator missing")
    ...
    if requestItem then
      ...
      if isTagBlacklisted then
        ...
        if whitelistException then
          local bridgeCount = (bridgeItem and (bridgeItem.amount or bridgeItem.count)) or 0
      ...
      elseif gearName then
        ...
        if gearStock and (gearStock.amount or gearStock.count or 0) > 0 then
